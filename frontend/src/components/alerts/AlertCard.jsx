/**
 * AlertCard
 * Componente presentacional para una alerta o evento de seguridad individual.
 * Renderiza la información de detección proveniente de YOLO/FastAPI.
 * * @param {Object} props
 * @param {Object} props.alert - Objeto de alerta
 * @param {string} props.alert.id - ID único del evento
 * @param {string} props.alert.label - Descripción de la detección (ej. "Persona", "Vehículo")
 * @param {string} props.alert.timestamp - Fecha y hora ISO 8601
 * @param {string} props.alert.cameraId - ID de la cámara origen
 * @param {number} props.alert.confidence - Nivel de confianza de la IA (0 a 1)
 * @param {'low'|'medium'|'high'|'critical'} [props.alert.riskLevel] - Nivel de riesgo opcional
 */
const AlertCard = ({ alert }) => {
  // Validación defensiva
  if (!alert) return null;

  // Formateo de hora estilo VMS (Técnico, 24h)
  ...
  // Mapeo dinámico de severidad basado en el riesgo (o fallback estandar)
  const getRiskStyles = (level) => {
    ...
    // Fallback: Si el backend de FastAPI no envía riskLevel, asume un estilo estándar (High/Orange) para detecciones de IA.
  ...
  return (
    <div className={`flex flex-col bg-[#1e1e1e] border-l-4 ${riskStyles.border} hover:bg-[#252526] transition-colors p-2.5 cursor-pointer group shadow-sm`}>
      
      {/* Cabecera: Detección, Nivel y Timestamp */}
      ...
      {/* Footer de la tarjeta: Metadatos de la cámara y acciones */}
      <div className="flex justify-between items-end mt-0.5">
        ...
        {/* Acción rápida (preparada para abrir un modal de evidencia) */}
        <button 
          ...
          title="Ver snapshot de evidencia"
        >
